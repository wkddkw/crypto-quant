...
**每钱包拉取**：近 **90 天**、单笔 `trade_usd >= 1500` 的买入事件。
阈值依据所有者判定（2026-09-02）：top-100 大户的 $200 级买入多为测试仓
或零碎操作，不构成其真实意图的信号；$1500 为最低有效信号门槛。
...
**目标规模**：按每钱包每周 5–10 笔 ≥$1.5k 买入估算约 0.7–1.3 万事件；
超过 2 万说明阈值执行不严，先排查再分析。
...
- 报告维度：全体分布、**按 rank 分层**（top-20 / 21-50 / 51-100）、**按单笔金额分层**（1.5–5k / 5–20k / 20–100k / >100k 美元）、按钱包聚合。
- 判据指标：
