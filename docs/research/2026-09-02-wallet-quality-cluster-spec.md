...
- **暴露度分组**（与 M3b 联动）：公开主号买入 vs 未曝光影子子号买入，
  分别给出衰减斜率。预期前者更陡；若两组无差，说明平台信息扩散均匀，
  "盯影子号"的立足点不成立。
...
### M3b — 影子钱包扩展（从 E1 到监控面）

E1 的资金溯源边不只用于去重，反向使用即得**监控扩展**：从每个榜单主钱包
出发，沿大额转出（排除合约/CEX 热钱包，复用 E1 的反向遍历）发现 1–2 跳
的衍生地址，作为该簇的观察成员加入采集面。动机（2026-09-02 案例）：
公开排行榜上的主号买入会被全平台复制者秒见、延迟折价最重；无人跟踪的
影子子号才更接近真实意图。

- 影子成员**不计入** 100 钱包的 L1 独立簇计数（同簇一票），但纳入采集、
  M1 收益分层与 L2 观察。
- **曝光衰减标记**：成员进入监控面时记录"首次公开传播时间"（如被喊单帖
  公开点名）。影子地址一经传播其优势即失效——被点名的地址单独分组测
  延迟折价，验证"曝光→衰减"假设。
- 衍生集合每次榜单快照（§2）重建，禁止硬编码。
